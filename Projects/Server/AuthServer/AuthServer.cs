...
namespace AuthServer
{
    class AuthServer
    {
        static void Main(string[] args)
        {
            ...
            Log.Message(LogType.Init, "_________________AuthServer_________________");
            Log.Message();

            new Server("127.0.0.1", 23115);
            PacketManager.DefineMessageHandler();

            Log.Message(LogType.Normal, "AuthServer listening on {0} port {1}.", "127.0.0.1", 23115);
